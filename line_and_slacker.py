...
class Element:
    def __init__(self, x, y, maxt=10000):
        self.x = x
        self.y = y
        ...
        self.t = 0  # tension (used for color)
        self.maxt = maxt  # max tension (for colorscale)
...
class Anchor(Element):
    def start_connection(self, next):
        next.xp = self.x
        next.yp = self.y

    def end_connection(self, previous):
        previous.xn = self.x
        previous.yn = self.y
...
class Segment:
    def __init__(self, main_webbing, main_length, backup_webbing, backup_length, np):
        self.main = main_webbing
        self.main_l = main_length
        self.backup = backup_webbing
        self.backup_l = backup_length
        self.np = np  # number of points

    def describe(self):
        return f'Main:{self.main_l}m of ' + self.main.describe() \
               + f' Backup:{self.backup_l}m of ' + self.backup.describe()
